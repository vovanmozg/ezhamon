...
function googleGetPR($aUrl)
{
    ...
    //$pr = ($pr!="" || $pr!=" ")?'x':$pr;
		if(preg_match("/Rank_1:(.):(.+?)\n/is", $pr_str, $res)) { return "$res[2]"; }
    else return 'x';
    //return $pr;
} 

function googleGetLinksCount($url){
	...
	if(preg_match("!of (about )?<b>(.*?)</b> linking!si",$file,$ok)){
		...
		if($ok[1] == "about "){
			...
			if(preg_match("!did not match any documents!si",$file)){
				$link="0";
			}
			else {
				$link="x";
...
function googleGetPagesCount($url){
	...
	if(preg_match("!of (about )?<b>(.*?)</b> from!si",$file,$ok)){
		...
		if($ok[1] == "about "){
			...
			if(preg_match("!No results found!si",$file)){
				$link="0";
			}
			else {
				$link="x";
